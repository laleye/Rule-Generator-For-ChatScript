...
class RuleGenerator(object):
    ...
    def __init__(self):
        self.fre_stopwords = get_stopswords()
        self.SPEC = ["'", "~"]
        self.RE_STRIP_REFS = re.compile("\.?\[\d+\]?")
    ...
    def interjection_concepts(self, filename):
    ...
    def build_concepts(self, filename):
        concepts_words = dict()
        words_concepts = dict()
        with open(filename, 'r', encoding='utf8') as f:
            for line in f:
        ...
        with open('models/concepts_words.pickle', 'wb') as handle:
            pickle.dump(concepts_words, handle, protocol=pickle.HIGHEST_PROTOCOL)
        with open('models/words_concepts.pickle', 'wb') as handle:
            pickle.dump(words_concepts, handle, protocol=pickle.HIGHEST_PROTOCOL)
            
        with open("models/donn.txt", "a", encoding='utf8') as f:
            for key, items in concepts_words.items():
                f.write("{}\t{}\n\n".format(key, items))
        
            
    def QA_File(self, filename):
        """
        Transform a file containing the answers following the questions into a file with the answers and questions separated by a tabulation
        """
        ...
        for i, text in enumerate(qa['questions']):
            with open('questions.txt', 'a', encoding='utf8') as out_:
                out_.write("{}\t{}\n".format(text, qa['responses'][i]))
                
            
    
    
    def load_interjections_concepts(self):
        with open('models/interjections.pickle', 'rb') as handle:
            interjections = pickle.load(handle)
        with open('models/concepts_words.pickle', 'rb') as handle:
            concepts_words = pickle.load(handle)
        with open('models/words_concepts.pickle', 'rb') as handle:
    ...
    def replace_interjections(self, question, interjections):
    ...
    def process_inter_concepts(self, inter_concepts):
        internals = list()
        internals_dict = dict()
    ...
    def replace_concepts(self, question, concepts_words, words_concepts):
    ...
    def replace_with_lemma(self, word, interjections, concepts_words, words_concepts):
        #word = lima.sendAndReceiveLima(word, mode="lemma")
        doc = nlp(wword)
        word_ = [token.lemma_ for token in doc]
        word = word_[0]
        #print(word)
    ...
    def seperate_subject(self, word):
    ...
    def generete_rules(self, qa, interjections, concepts_words, words_concepts):
        questions = list()
        concept_dict = dict()
        ...
        for q in questions:
            ...
            with open("chatscript_rules.top", "a", encoding='utf8') as f:
                f.write("#! {}\n".format(q[0]))
                f.write("{}\n".format(rule))
...
if __name__ == "__main__":
    ...
    qafile = "@CMAKE_INSTALL_PREFIX@/g5.txt"
